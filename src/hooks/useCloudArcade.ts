...
export function useCloudArcade(options: CloudArcadeOptions = {}) {
  ...
  const inIframe = useRef(isInIframe());
  ...
  // Send message to parent (only if in iframe)
  const sendMessage = useCallback((message: GameToParentMessage) => {
    if (!inIframe.current) {
      log('Skipping (not in iframe):', message.type);
      return;
    }
    log('Sending:', message);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      // Ignore messages from same window (prevents echo)
      if (event.source === window) return;
      
      const data = event.data as ParentToGameMessage;
      if (!data || typeof data !== 'object' || !('type' in data)) return;
      
      // Only process known parent message types
      const validTypes = ['USER_INFO', 'SESSION_STARTED', 'SESSION_ENDED', 'SCORE_SUBMITTED', 'SCORE_ERROR'];
    ...
    // Send GAME_READY only once
    if (!gameReadySentRef.current) {
  ...
  return {
    isConnected: state.isPlatformConnected,
    startSession,
    endSession,
    submitScore,
    gameOver,
